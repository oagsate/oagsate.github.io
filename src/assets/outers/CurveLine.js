...
var scales = [10000000, 5000000, 2000000, 1000000, 500000, 200000, 100000, 50000, 25000, 20000, 10000, 5000, 2000, 1000, 500, 200, 100, 50, 20];
...
(function () {
   ...
   * 根据弧线的坐标节点数组
   */
  function getCurvePoints(points, zoom) {
    var curvePoints = [];
    var map = new BMap.Map();
    var count = 30; // 曲线是由一些小的线段组成的，这个表示这个曲线所有到的折线的个数
    for (var i = 0; i < points.length - 1; i++) {
      if (zoom) {
        const distance = map.getDistance(points[i], points[i + 1]);
        count = distance / scales[zoom - 1] * 20;
      }

      var p = getCurveByTwoPoints(points[i], points[i + 1], count);
      if (p && p.length > 0) {
  ...
  CurveLine.getCurvePoints = getCurvePoints;
